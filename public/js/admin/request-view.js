...
const RequestViewThin = (function () {
    ...
    const renderActionButtons = (data) => {
        ...
        document
            ...
            ?.addEventListener("click", () => {
                document.getElementById("approvalCountFinalize").textContent =
                    approvalInfo.approval_count;
                document.getElementById("rejectionCountFinalize").textContent =
                    approvalInfo.rejection_count;
                new bootstrap.Modal(
    ...
    const renderDetails = (data) => {
        ...
        // LEFT CARD: Request Information (no Total Fee)
        document.getElementById("detailsContainer").innerHTML = `
        <div class="col-md-6">
            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-muted">Requester</span>
                <span class="fw-medium">${escapeHtml(user.first_name)} ${escapeHtml(user.last_name)}</span>
            </div>
            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-muted">School ID</span>
                <span class="fw-medium">${escapeHtml(user.school_id || "N/A")}</span>
            </div>
            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-muted">Email</span>
                <span class="fw-medium">${escapeHtml(user.email)}</span>
            </div>
            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-muted">Organization</span>
                <span class="fw-medium">${escapeHtml(user.organization_name || "N/A")}</span>
            </div>
            <div class="d-flex justify-content-between py-2">
                <span class="text-muted">Contact</span>
                <span class="fw-medium">${escapeHtml(user.contact_number || "N/A")}</span>
            </div>
        </div>
        <div class="col-md-6">
            <div class="d-flex justify-content-between py-2 border-bottom">
            ...
            <div class="d-flex justify-content-between py-2">
                ...
                <span class="fw-medium">${data.duration_hours} hours${data.is_multi_day ? " (multi-day)" : ""}</span>
            </div>
        </div>
        ${
            ...
            <div class="col-12 mt-3">
                <div class="fw-bold text-primary mb-1">Additional Requests</div>
                <div class="text-muted">${escapeHtml(form.additional_requests)}</div>
            </div>
        `
                : ""
        }
    `;
        ...
        // RIGHT CARD: Booking Details (with Total Fee at top)
        ...
        ${
            data.requested_items.facilities.length
                ? `
            <div class="mb-3">
                <div class="fw-bold text-primary small">Facilities</div>
                ${data.requested_items.facilities
                    .map(
                        (f) =>
                            `<div class="d-flex justify-content-between small">
                        <span>${escapeHtml(f.name)}</span>
                        <span class="${f.is_waived ? "text-muted text-decoration-line-through" : ""}">${formatMoney(f.total_fee)}</span>
                    </div>`,
                    )
                    .join("")}
            </div>
        ...
        ${
            data.requested_items.equipment.length
                ? `
            <div class="mb-3">
                <div class="fw-bold text-primary small">Equipment</div>
                ${data.requested_items.equipment
                    .map(
                        (e) =>
                            `<div class="d-flex justify-content-between small">
                        <span>${escapeHtml(e.name)} ${e.quantity > 1 ? `×${e.quantity}` : ""}</span>
                        <span class="${e.is_waived ? "text-muted text-decoration-line-through" : ""}">${formatMoney(e.total_fee)}</span>
                    </div>`,
                    )
                    .join("")}
            </div>
